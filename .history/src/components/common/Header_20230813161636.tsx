...
const Header = ({ myInfo, isAuthorized, countInfos, onLogout }: Props) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      let now = countInfos.map((countInfos) => countInfos.todayVar) as number[];
      let now2 = countInfos.map(
        (countInfos) => countInfos.totalVar,
      ) as number[];

      const handle = setInterval(() => {
        setCounter(
          (counterRef.innerHTML = Math.ceil(
            (countInfos.map((countInfos) => countInfos.todayVar) as number[]) -
              now,
          )
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',')),
        ...
        setTcounter(
          (tcounterRef.innerHTML = Math.ceil(
            (countInfos.map((countInfos) => countInfos.totalVar) as number[]) -
              now2,
          )
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ',')),
